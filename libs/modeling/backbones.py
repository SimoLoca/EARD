...
from .blocks import (get_sinusoid_encoding, TransformerBlock, MaskedConv1D,
                     ConvBlock, LayerNorm, LTSBlock)


@register_backbone("LongToShort")
class LongToShort(nn.Module):
    def __init__(
        self,
        ...
        for idx in range(arch[1]):
            self.stem.append(
              LTSBlock(
                  n_embd, kernel_size=1, stride=1, dilation=1, path_pdrop=0
              )
            )
        ...
        for idx in range(arch[2]):
            ...
            self.branch.append(
                LTSBlock(
                    n_embd, kernel_size=win_size[idx], stride=2, dilation=dilation, path_pdrop=path_pdrop
                )
            )
    ...
    def forward(self, x, mask):
        ...
        # main branch with downsampling & Residual-like Pyramid
        for idx in range(len(self.branch)):
            x, mask = self.branch[idx](x, mask, out_feats)
            out_feats += (x, )
            out_masks += (mask, )
